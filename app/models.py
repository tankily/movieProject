...
class User(db.Model):
    ...
    userlogs = db.relationship('Userlog', backref='user')  # 外键关系关联日志
    comments = db.relationship('Comment', backref='user')  # 外键关系评论
    moviecols = db.relationship('Moviecol', backref='user')  # 外键关联电影收藏
...
class Userlog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)  # 编号
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"))  # 与user表主键关联 所属会员
...
class Movie(db.Model):
    ...
    moviecols = db.relationship("Moviecol", backref='movie')  # 外键关联电影收藏
...
class Admin(db.Model):
    ...
    name = db.Column(db.String(100), unique=True)  # 昵称
    pwd = db.Column(db.String(100))  # 密码
    is_super = db.Column(db.SmallInteger)  # 是否为超级用户，0为超级用户
    role_id = db.Column(db.Integer, db.ForeignKey('role.id'))  # 所属角色
    addtime = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    adminlogs = db.relationship("Adminlog", backref='admin')  # 管理员登陆日志关联
